...
export default async function ProjectFinanceChequesPage({ params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions);
  const companyId = (session?.user as any)?.companyId ?? '';
  const project = await prisma.project.findFirst({ where: { id: params.id, companyId }, select: { id: true, name: true } });
  if (!project) notFound();

  const { cheques, totals } = await getChequeSummary(companyId, project.id);
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-base font-semibold">Project Cheques</h1>
          <p className="text-xs text-muted-foreground">{project.name}</p>
        </div>
        <Link href={`/projects/${project.id}/finance/cash-bank`} className="text-xs text-primary hover:underline">Cash / Bank Book</Link>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/40">
              ...
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase">Cheque Date</th>
          ...
          <tbody className="divide-y">
            {cheques.map((cheque) => (
              <tr key={cheque.id}>
                ...
                <td className="px-4 py-3 text-right">{cheque.status === 'PENDING' ? <ChequeStatusActions chequeId={cheque.id} /> : <span className="text-xs text-muted-foreground">Locked</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
